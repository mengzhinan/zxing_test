...
final class VCardFieldFormatter implements Formatter {
    ...
    private static CharSequence formatMetadata(CharSequence value, Map<String, Set<String>> metadata) {
        ...
        if (metadata != null) {
            for (Map.Entry<String, Set<String>> metaDataItem : metadata.entrySet()) {
                Set<String> values = metaDataItem.getValue();
                if (values == null || values.isEmpty()) {
                    continue;
                }
                withMetadata.append(';').append(metaDataItem.getKey()).append('=');
                if (values.size() > 1) {
                    withMetadata.append('"');
                }
